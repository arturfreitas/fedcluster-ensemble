...
import random
from niid_bench.control_utils import optimize_fraction_fit_moo


from niid_bench.utils import calculate_similarity, calculate_trend,find_clusters, experiment_with_dbscan_and_print
...
class FedMPC(FedAvg):
    def __init__(self, fraction_fit: float = 1.0, *args, **kwargs):
        self.num_rounds = kwargs.pop('num_rounds', 60)
        super().__init__(*args, **kwargs)
        self.fraction_fit = fraction_fit
        self.last_global_weights = None
        self.cluster_label_to_model_map = {}
        self.cid_cluster_map = {}
        self.performancedataset = {}
        self.last_accuracy = None
        self.last_fraction_fit = None
        self.predicted_accuracy = None
        # self.predicted_fraction_fit = None not needed since it is already assigned to fraction_fit
    ...
    def evaluate(self, server_round: int, parameters: Parameters) -> Optional[Tuple[float, Dict[str, Scalar]]]:
        ...
        # ✅ Log and update historical data for optimization
        self.performancedataset[server_round] = {
            "round": server_round,
            "fraction_fit": self.fraction_fit,
            "fraction_fit_prev": self.last_fraction_fit,
            "centralized_accuracy_prev": self.last_accuracy,
            "centralized_accuracy": accuracy,
            "predicted_accuracy": self.predicted_accuracy
        }

         # After updating self.performancedataset[server_round]...
        window_size = 5
        if server_round >= window_size:
            # Compute trend on the last `window_size` entries
            trend = calculate_trend(self.performancedataset, metric='centralized_accuracy', window=window_size)
        else:
            trend = None
    
        self.performancedataset[server_round]["accuracy_trend"] = trend
        
        logging.info(
                    f"[Round {server_round}] accuracy = {accuracy:.4f} (prev = {self.last_accuracy}), "
                    f"fraction_fit = {self.fraction_fit:.4f} (prev = {self.last_fraction_fit})"
                    f"trend = {trend}"
                )
        ...
        self.last_fraction_fit = self.fraction_fit

        metrics.update(self.performancedataset[server_round])
    ...
    def configure_fit(
        ...
        # ✅ Only optimize if we have valid history
        if server_round > 5 and self.last_accuracy is not None and self.last_fraction_fit is not None:
            best_plan, best_acc, best_frac = optimize_fraction_fit_moo(
                round_now=server_round,
                performance_dataset=self.performancedataset, 
                h=5,
                n_trials=200,
                alpha=0.5,
                beta=0.5
        ...
        if best_plan is not None:
            self.fraction_fit = best_plan[0]
            self.predicted_accuracy = best_acc  # Agora é apenas do round seguinte
        else:
            self.fraction_fit = 0.7

        logging.info(f"[Round {server_round}] fraction_fit = {self.fraction_fit:.3f}")

        # Sample clients
        available_clients = client_manager.all()
        num_clients_to_sample = max(1, int(self.fraction_fit * len(available_clients)))
        sampled_clients = client_manager.sample(
            num_clients=num_clients_to_sample,
    ...
    def save_dataset(self, filename="fedavg_training_log3_descent.csv", output_dir="results"):
        os.makedirs(output_dir, exist_ok=True)
        path = os.path.join(output_dir, filename)
        df = pd.DataFrame.from_dict(self.performancedataset, orient="index")
        df = df.sort_values("round")
        df.to_csv(path, index=False)
        logging.info(f"[FedAvgExp] Training log saved to {path}")
