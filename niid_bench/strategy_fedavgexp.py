...
from niid_bench.utils import calculate_similarity,find_clusters, experiment_with_dbscan_and_print, calculate_trend
...
class FedAvgExp(FedAvg):
    ...
    def configure_fit(
        ...
        config={}
        # Define total number of rounds and minimum fraction
        total_rounds = 60
        min_fraction = 0.05
        max_fraction = 1.0

        # Linearly decrease fraction_fit from max_fraction to min_fraction
        if server_round <= total_rounds:
            self.fraction_fit = max_fraction - ((max_fraction - min_fraction) * (server_round - 1) / (total_rounds - 1))
        else:
            self.fraction_fit = min_fraction  # Hold steady after ramp ends
