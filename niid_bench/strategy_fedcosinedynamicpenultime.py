...
class FedCosineDynamicPenultime(FedAvg):
    ...
    def configure_fit(
        ...
        # Linear decrease formula for the number of clients
        start_clients = len(clients)
        end_clients = 2
        total_rounds = 30
        
        # Calculate alfa (the decrement per round)
        alfa = (start_clients - end_clients) / total_rounds
        
        # Calculate the number of clients for the current round
       
        num_clients = int(start_clients - alfa * (server_round - 1))
        logging.info(f'Number of clients for rouind {server_round} is {num_clients}')
        ...
        if server_round == 1:
            ...
            selected_clients = [client for client, _ in self.clients_with_similarities[:num_clients]]    
        
        logging.info(f'length of selected clients on {server_round} is {len(selected_clients)}')
    ...
    def aggregate_fit(
        ...
        for client, fit_res in results:
            ...
            similarity = calculate_similarity(self.last_global_weights[-2], client_weights[-2])
            self.clients_with_similarities.append((client, similarity))
            # Sort clients by similarity
            self.clients_with_similarities.sort(key=lambda x: x[1], reverse=True)
            
        for client, sim in self.clients_with_similarities:
