...
class FedDissimilarityBiased(FedAvg):
    ...
    def configure_fit(
        ...
        if server_round == 1:
            ...
            # Invert shifted similarities to prioritize clients with higher dissimilarity
            inverted_similarities = 1.0 / (shifted_similarities + 1e-10)  # Small value to avoid division by zero
            probabilities = np.exp(inverted_similarities) / np.sum(np.exp(inverted_similarities))
            
            # Apply softmax to calculate probabilities for client selection
            exp_inverted_similarities = np.exp(inverted_similarities)
