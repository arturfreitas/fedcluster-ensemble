...
class FlowerClientFedMedian(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config: Dict[str, Scalar]):
        ...
        if server_round > 0:
            sel_thresh = config["selection_thrshold"]
            
            logging.info(f'selection trhesholf for round {server_round} is: {sel_thresh}')
            
            if similarity < sel_thresh:
                return_dict["data_transferred"] = 0
                return [], len(self.trainloader), return_dict
...
def gen_client_fn(
...
) -> Callable[[str], FlowerClientFedMedian]:  # pylint: disable=too-many-arguments
    ...
    def client_fn(cid: str) -> FlowerClientFedMedian:
